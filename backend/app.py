...
from services.email_service import send_candidate_email # For the email route
# Import blueprints for routes
from routes.upload_routes import upload_bp
from routes.candidate_routes import candidate_bp
from routes.jd_routes import jd_bp
from routes.interview_routes import interview_bp # NEW
...
# --- NLTK & SpaCy Downloads (run once at app startup) ---
# It's good practice to ensure these are downloaded in the main app setup
...
app.register_blueprint(interview_bp) # NEW: Register the interview blueprint

# --- Email Sending API (remains directly in app.py or could be a separate blueprint) ---
@app.route('/send_email', methods=['POST'])
def send_email_route():
    ...
    try:
        send_candidate_email(recipient_email, subject, body)
        return jsonify({"status": "success", "message": "Email sent successfully!"}), 200
    except Exception as e:
        print(f"Error sending email: {e}")
        return jsonify({"error": f"Failed to send email: {str(e)}"}), 500
...
@app.route('/interview/slots', methods=['GET']) # <-- THIS IS THE CRUCIAL LINE
def get_interview_slots():
    job_id = request.args.get('job_id')
    if not job_id:
        return jsonify({"message": "job_id is required"}), 400

    # --- DUMMY LOGIC FOR GENERATING SLOTS (as provided previously) ---
    slots = []
    now = datetime.datetime.now()
    for i in range(1, 6):
        start_time = now + datetime.timedelta(days=i, hours=10, minutes=0)
        end_time = start_time + datetime.timedelta(hours=1) # 1-hour interview
        slots.append({
            "id": str(uuid.uuid4()), # Unique ID for the slot
            "startTime": start_time.isoformat(),
            "endTime": end_time.isoformat(),
            # Make sure INTERVIEWERS_DB is defined or you use real data
            "interviewer": {"id": "dummy_interviewer", "name": "Dummy Interviewer"}
        })
    # --- END DUMMY LOGIC ---
